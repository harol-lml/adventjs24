...
function minMovesToStables(reindeer, stables) {
    let stalesCopy = stables.slice()
    let rem = reindeer.map((reind) => {
        let band = 100000, num  = 0

        stalesCopy.forEach((stable, ind) => {
            let pla = Math.abs(stable - reind)
            if( pla < band){
                band = pla
                num  = ind
            }
        })
        stalesCopy.splice(num, 1)
